      ...
      <header className="sticky top-0 z-50 border-b border-white/[0.06] bg-background/80 backdrop-blur-xl">
        <div className="max-w-7xl mx-auto flex items-center justify-between px-6 py-4">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-lg font-semibold text-white">Predictive AI</h1>
          </div>
          ...
          <div className="flex items-center gap-1 p-1 rounded-xl bg-white/[0.04] border border-white/[0.06]">
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all duration-200
                ${activeTab === "fraud"
                  ? "bg-gradient-to-r from-red-500/20 to-orange-500/20 text-red-300 border border-red-500/20"
            ...
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all duration-200
                ${activeTab === "forecast"
                  ? "bg-gradient-to-r from-violet-500/20 to-cyan-500/20 text-violet-300 border border-violet-500/20"
                  : "text-slate-400 hover:text-slate-200"
                }`}
            >
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
      ...
      <div className="space-y-6">
        ...
        <div className="space-y-4 p-6 rounded-2xl border border-white/[0.06] bg-white/[0.02]">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-3 py-2 rounded-lg bg-white/[0.04] border border-white/[0.08] text-white text-sm
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-xs font-medium text-slate-400 mb-1.5">Hour (0-23)</label>
              <input
                type="number"
                min={0}
                max={23}
      ...
      <div className="space-y-6">
        {result ? (
          ...
          <div className="flex flex-col items-center justify-center h-64 rounded-2xl border border-white/[0.06] bg-white/[0.02]">
            <Shield className="w-12 h-12 text-slate-600 mb-3" />
            <p className="text-slate-500 text-sm">Enter transaction details and click Analyze</p>
          </div>
        )}
        ...
        {stats && !statsLoading && (
          <div className="space-y-4">
            <h3 className="text-lg font-semibold text-white">Fraud Landscape</h3>
            {/* KPI Row */}
            ...
            <div className="p-4 rounded-2xl border border-white/[0.06] bg-white/[0.02]">
              <p className="text-xs font-medium text-slate-400 mb-3">Fraud Rate by Bank</p>
              <ResponsiveContainer width="100%" height={180}>
                <BarChart data={stats.by_bank} layout="vertical" margin={{ left: 60 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.04)" />
                  <XAxis type="number" tick={{ fill: "#94a3b8", fontSize: 11 }} />
                  <YAxis dataKey="bank" type="category" tick={{ fill: "#cbd5e1", fontSize: 11 }} width={55} />
                  <Tooltip
  ...
  return (
    ...
    >
      ...
      <div className={`p-6 rounded-2xl border ${
        isHigh ? "border-red-500/30 bg-red-500/[0.05]" :
        isMedium ? "border-orange-500/30 bg-orange-500/[0.05]" :
        "border-emerald-500/30 bg-emerald-500/[0.05]"
      }`}>
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            {isHigh ? <AlertTriangle className="w-6 h-6 text-red-400" /> :
             isMedium ? <AlertTriangle className="w-6 h-6 text-orange-400" /> :
             <CheckCircle className="w-6 h-6 text-emerald-400" />}
            <div>
              <p className="text-lg font-bold text-white">{result.prediction}</p>
              <p className={`text-xs font-medium ${
                isHigh ? "text-red-400" : isMedium ? "text-orange-400" : "text-emerald-400"
              }`}>
          ...
          <div className="text-right">
            <p className="text-3xl font-black text-white">{probability}%</p>
      ...
      {shapEntries.length > 0 && (
        <div className="p-5 rounded-2xl border border-white/[0.06] bg-white/[0.02]">
          <div className="flex items-center gap-2 mb-4">
            ...
            <p className="text-sm font-semibold text-white">SHAP Feature Impact</p>
          </div>
            ...
            {shapEntries.map(([feature, value]) => {
              ...
              return (
                <div key={feature} className="flex items-center gap-3">
                  <span className="text-xs text-slate-400 w-32 truncate text-right">{feature}</span>
                  <div className="flex-1 flex items-center gap-2">
                    <div className="flex-1 h-4 rounded bg-white/[0.04] overflow-hidden relative">
                      <motion.div
                        ...
                        className={`h-full rounded ${isPositive ? "bg-red-500/60" : "bg-emerald-500/60"}`}
                      />
                    </div>
                    <span className={`text-xs font-mono w-14 text-right ${isPositive ? "text-red-400" : "text-emerald-400"}`}>
...
const ForecastView = () => {
  ...
  if (!data) {
    return (
      <div className="flex flex-col items-center justify-center h-96 rounded-2xl border border-white/[0.06] bg-white/[0.02]">
        <TrendingUp className="w-12 h-12 text-slate-600 mb-3" />
        <p className="text-slate-500 text-sm">Forecast data not available</p>
        <p className="text-slate-600 text-xs mt-1">Run: python scripts/train_forecast_model.py</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-white mb-1">30-Day Volume Forecast</h2>
          <p className="text-sm text-slate-400">
        ...
        <div className="flex items-center gap-1 p-1 rounded-lg bg-white/[0.04] border border-white/[0.06]">
          <button
            ...
            className={`px-3 py-1.5 rounded-md text-xs font-medium transition-all ${
              metric === "count" ? "bg-violet-500/20 text-violet-300" : "text-slate-400"
            }`}
          >
            Txn Count
          </button>
          <button
            ...
            className={`px-3 py-1.5 rounded-md text-xs font-medium transition-all ${
              metric === "amount" ? "bg-cyan-500/20 text-cyan-300" : "text-slate-400"
            }`}
          >
            Txn Amount
          </button>
      ...
      <div className="p-6 rounded-2xl border border-white/[0.06] bg-white/[0.02]">
        <div className="flex items-center gap-4 mb-4">
          ...
          <div className="flex items-center gap-2">
            <div className="w-6 h-3 rounded bg-cyan-500/20" />
        ...
        <ResponsiveContainer width="100%" height={380}>
          <ComposedChart data={chartData} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
            ...
            <Area dataKey="lower" stroke="none" fill="rgba(15, 23, 42, 1)" />
            {/* Historical */}
            <Line
              dataKey="actual"
              stroke="#8b5cf6"
  ...
  <div>
    ...
    <div className="relative">
      <select
        ...
        className="w-full px-3 py-2 rounded-lg bg-white/[0.04] border border-white/[0.08] text-white text-sm
                   appearance-none cursor-pointer focus:outline-none focus:border-violet-500/50 transition-colors"
      >
        {options.map((opt) => (
...
const KpiCard = ({
  ...
  const colorMap: Record<string, string> = {
    red: "from-red-500/10 to-red-500/5 border-red-500/10 text-red-300",
    orange: "from-orange-500/10 to-orange-500/5 border-orange-500/10 text-orange-300",
    cyan: "from-cyan-500/10 to-cyan-500/5 border-cyan-500/10 text-cyan-300",
    violet: "from-violet-500/10 to-violet-500/5 border-violet-500/10 text-violet-300",
    emerald: "from-emerald-500/10 to-emerald-500/5 border-emerald-500/10 text-emerald-300",
    amber: "from-amber-500/10 to-amber-500/5 border-amber-500/10 text-amber-300",
  };
  const classes = colorMap[color] || colorMap.violet;

  return (
    <div className={`p-4 rounded-xl bg-gradient-to-br border ${classes}`}>
